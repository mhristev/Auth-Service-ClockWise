package com.clockwise.authService.exception
...
class AuthExceptionHandler {
    ...
    fun handleIllegalArgument(ex: IllegalArgumentException, request: ServerHttpRequest): ResponseEntity<ErrorResponse> {
        logger.warn { "Authentication/Authorization error: ${ex.message} for path: ${request.uri.path}" }
        val errorResponse = ErrorResponse(
            status = HttpStatus.UNAUTHORIZED.value(),
            error = "Unauthorized",
            message = ex.message ?: "Invalid credentials or token",
    ...
    fun handleSecurity(ex: SecurityException, request: ServerHttpRequest): ResponseEntity<ErrorResponse> {
        logger.warn { "Security error: ${ex.message} for path: ${request.uri.path}" }
        val errorResponse = ErrorResponse(
